## This is the script for plot 4 of the Module 1 project of the Explanatory Analysis with R class.
...
## Finally, we create plot 4, which is a combination of plots 2 and 3 and 2 new ones whose formats
## are similar to that of plot 2, and export it as PNG.
par(mfrow = c(2, 2))

## I also gave the bottom a little more margin to breathe and compensated with the top.
par(mar=c(6,5,2,2))

## Plot 4.1 (plot 2 with a slightly shorter y-axis label)
plot(power$DateTime, power$Global_active_power, type = "l", lty=1, xaxt = "n",
     xlab = "", ylab="Global Active Power")
axis.POSIXct(1, at = as.Date(power$DateTime), format = "%a")

## Plot 4.2
plot(power$DateTime, power$Voltage, type = "l", lty=1, xaxt = "n",
     xlab = "datetime", ylab="Voltage")
axis.POSIXct(1, at = as.Date(power$DateTime), format = "%a")

## Plot 4.3 (plot 3 with a slightly smaller legend with no visible box)
plot(power$DateTime, power$Sub_metering_1, type = "l", xaxt = "n", xlab="", ylab="Energy sub metering")
lines(power$DateTime, power$Sub_metering_2, col="red")
lines(power$DateTime, power$Sub_metering_3, col="blue")
legend("topright", legend = c("Sub_metering_1           ","Sub_metering_2           ","Sub_metering_3           "), 
       cex= 0.9, lty = 1, lwd=1, col= c("black","red", "blue"), xpd=TRUE, bty="n")
...
## Plot 4.4
plot(power$DateTime, power$Global_reactive_power, type = "l", lty=1, xaxt = "n",
...
## Finally, we reset our general parameters so as not to be bothered if we decide to plot something else.
